...
const REG_DISABLED_ALIASES: *const u16 = w!("DisabledAliases");
...
pub fn uumain<T: Args>(args: T) -> UResult<()> {
    ...
    match matches.subcommand() {
        ...
        Some(("refresh", matches)) => {
            if !ensure_elevated(matches, "refresh", &BTreeSet::new())? {
                return Ok(());
            }
            sync_install(&utilities, &disabled)
        }
...
fn sync_install(utility_names: &BTreeSet<&str>, disabled: &BTreeSet<String>) -> UResult<()> {
    ...
    {
        sync_alias_links(utility_names, disabled, &coreutils_exe, &bin_dir, "exe")?;
        sync_alias_links(utility_names, disabled, &coreutils_exe, &cmd_dir, "cmd")?;
        sync_alias_link(
            "coreutils-manager",
            disabled,
            &coreutils_exe,
...
fn sync_alias_links(
    ...
    for &utility_name in utility_names {
        sync_alias_link(utility_name, disabled, coreutils_path, dest_dir, suffix)?;
    }
    Ok(())
}
...
fn read_disabled_aliases() -> UResult<BTreeSet<String>> {
    OwnedHKEY::get_multi_sz_value(Registry::HKEY_LOCAL_MACHINE, REG_PATH, REG_DISABLED_ALIASES)
...
fn write_disabled_aliases(disabled: &BTreeSet<String>) -> UResult<()> {
    ...
    if disabled.is_empty() {
        let ret = unsafe { Registry::RegDeleteValueW(key.get(), REG_DISABLED_ALIASES) };
